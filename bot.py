...
async def process_start_command(message: types.Message):
    ...
    mark.add(button)
    await bot.send_message(message.chat.id, '\U0001F916 Ииии... Камера, мотор, погнали!', reply_markup=mark)
...
async def echo_message(msg: types.Message):
    ...
    if '111' in msg.text:
        ...
        await bot.send_message(msg.chat.id, 'Выбери город, чтобы узрить!', reply_markup=mark_weather)
...
async def weather_now(data, msg):
    ...
    weather_smile = data["weather"][0]["main"]
    if weather_smile in code_to_smile:
        weather_smile = code_to_smile[weather_smile]
        msg_bot = f'{code_to_smile[weather_smile]}\n'
        write_msg_bot(msg, msg_bot)
    else:
